...
class segment_template {
  ...
  public:
    ...
    friend bool operator==(segment_template const &a,
...
auto segment_template_rule_t::parse(char const *&it, char const *end)
  ...
  public:
    ...
    child_idx_vector(child_idx_vector const &other)
        ...
        if (other.child_idx) {
            ...
            std::memcpy(child_idx, other.child_idx,
...
class impl {
  ...
  public:
    ...
    void insert_impl(core::string_view path,
...
void impl::insert_impl(core::string_view path,
    ...
    while (it != end) {
        ...
        if (seg == "..") {
            ...
            if (cur == &nodes_.back() && !cur->resource &&
                cur->child_idx.empty()) {
                node *p = &nodes_[p_idx];
                std::size_t cur_idx = cur - nodes_.data();
                p->child_idx.erase(std::remove(p->child_idx.begin(),
                                               p->child_idx.end(), cur_idx));
                nodes_.pop_back();
            }
            cur = &nodes_[p_idx];
            ++it;
        ...
        if (cit != cur->child_idx.end()) {
            ...
            if (nodes_[cur_id].child_idx.size() > 1) {
                ...
                while (n) {
                    if (nodes_[cs.begin()[n]].seg <
                        nodes_[cs.begin()[n - 1]].seg)
        ...
        for (auto i : cur->child_idx) {
            ...
            if (c.seg.match(s)) {
                if (c.seg.is_literal()) {
                    ...
                    // next segment
                    if (branch) {
                        r = try_match(std::next(it), end, &c, level, matches,
                                      ids);
                        if (r)
                            break;
                    } else {
